...
// EmployeeDevice 员工终端设备，由 Desktop 系统同步，存储在 PostgreSQL。
// 注意区分：图中的 domain.Device 是网络设备（交换机/路由器），这里是员工的电脑/手机。
...
type EmployeeDeviceRepository interface {
	Save(ctx context.Context, dev *EmployeeDevice) error
	SaveBatch(ctx context.Context, devs []EmployeeDevice) error
	FindByExternalID(ctx context.Context, externalID string) (*EmployeeDevice, error)
	FindByEmployeeID(ctx context.Context, employeeID string) ([]EmployeeDevice, error)
...
func NewDeviceRepository(db *sqlx.DB) EmployeeDeviceRepository {
...
func (r *deviceRepo) Save(ctx context.Context, dev *EmployeeDevice) error {
	var lastOnline any
...
func (r *deviceRepo) FindByExternalID(ctx context.Context, externalID string) (*EmployeeDevice, error) {
	var row deviceRow
...
func (r *deviceRepo) FindByEmployeeID(ctx context.Context, employeeID string) ([]EmployeeDevice, error) {
	var rows []deviceRow
	...
	devs := make([]EmployeeDevice, 0, len(rows))
...
func (r *deviceRepo) SaveBatch(ctx context.Context, devs []EmployeeDevice) error {
...
func toDevice(row deviceRow) *EmployeeDevice {
