...
// NewEmployeeRepository 创建基于 PostgreSQL 的员工持久化实现。
func NewEmployeeRepository(db *sqlx.DB) identity.EmployeeRepository {
...
func (r *employeeRepo) Save(ctx context.Context, emp *identity.Employee) error {
...
func (r *employeeRepo) FindByExternalID(ctx context.Context, externalID string) (*identity.Employee, error) {
	var row employeeRow
...
func (r *employeeRepo) FindByID(ctx context.Context, id string) (*identity.Employee, error) {
	var row employeeRow
	err := r.db.GetContext(ctx, &row,
...
func (r *employeeRepo) FindAll(ctx context.Context) ([]identity.Employee, error) {
	...
	emps := make([]identity.Employee, 0, len(rows))
...
func (r *employeeRepo) SaveBatch(ctx context.Context, emps []identity.Employee) error {
...
func toEmployee(row employeeRow) (*identity.Employee, error) {
	var deptIDs []string
