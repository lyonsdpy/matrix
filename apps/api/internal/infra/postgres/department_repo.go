...
// NewDepartmentRepository 创建基于 PostgreSQL 的部门持久化实现。
func NewDepartmentRepository(db *sqlx.DB) identity.DepartmentRepository {
...
func (r *departmentRepo) Save(ctx context.Context, dept *identity.Department) error {
...
func (r *departmentRepo) FindByExternalID(ctx context.Context, externalID string) (*identity.Department, error) {
...
func (r *departmentRepo) FindAll(ctx context.Context) ([]identity.Department, error) {
	...
	depts := make([]identity.Department, 0, len(rows))
...
func (r *departmentRepo) SaveBatch(ctx context.Context, depts []identity.Department) error {
...
func toDepartment(row departmentRow) *identity.Department {
...
// nullStr 将空字符串转换为 nil（SQL NULL），非空字符串原样返回。
